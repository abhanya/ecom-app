...
def cart_login(request):
    ...
    if request.method == 'POST':
        cusername = request.POST['cusernm']
        cpas = request.POST['cstpass']

        try:
            customer = Customer.objects.get(customer_name = cusername, cust_pass = cpas)
            request.session['customer'] = customer.id
            return redirect('customer:chome')
        except:
            msg ='Username or passwordincorrect'
...
def signup(request):
    if request.method == 'POST':
        ...
        new_seller = Seller(seller_name = sell_name, seller_add = sell_add, seller_gen = sell_gen,
                    ...
                    branch = brnch, acc_num = acc_numb, email = s_email, seller_usr = s_username,
                    seller_pass = seller_pass, seller_pic = sellerimg)
...
def sellerlog(request):
    ...
    if request.method == 'POST':
        ...
        try:
            seller =Seller.objects.get(seller_usr = susername, seller_pass = spassword)
